...
void Player::begin()
{
    if(!isBusy)
    {
        ...
        playerProc->start("D:/Gui/3rdparty/ffmpeg-win64-gpl-shared/bin/ffplay.exe", QStringList() << "rtmp://58.200.131.2:1935/livetv/cctv1hd");
        this->start();
        isBusy = true;
...
void Player::slot_handle(quint64 handle,bool status)
{
    if(status)
    {
        ...
        widget->setGeometry(QRect(0,0,520,420));
